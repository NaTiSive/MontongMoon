...
export default function OwnerTreeStatus() {
  ...
  const trees = [
    ...
    { id: "T-032", status: "ออกดอก", note: "เริ่มบานตามกำหนด" },
    {
      id: "T-015",
      status: "มีปัญหา",
      note: "พบอาการใบไหม้ ต้องตรวจสวนเพิ่มเติม",
    },
  ...
  return (
    <div className="min-h-screen w-full bg-slate-50 text-slate-900 flex">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <HeaderWrapper title="สถานะต้นทุเรียนในสวน" subtitle="ติดตามสุขภาพต้นทุเรียนและวางแผนจัดการได้ตลอดเวลา" />
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {summary.map((s) => (
              <div key={s.label} className={`rounded-2xl shadow-sm bg-white`}>
                <div
                  className={`px-4 py-5 rounded-2xl ${s.cls.replace(
                    "text-",
                    "border-"
                  )} `}
                >
                  <div className={`text-sm ${s.cls.split(" ")[1]}`}>
                    {s.label}
                  </div>
